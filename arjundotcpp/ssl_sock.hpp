...
class ssl_sock_t : public sock_t
{
	...
	public:
		...
		read(std::vector< uint8_t >& data, std::size_t len = 8192)
		{
			...
			do {
				rlen = ::SSL_read(m_ssl, reinterpret_cast< char* >(&data[static_cast< int >(offset)]), static_cast< int >(len - offset));
		...
		write(std::vector< uint8_t >& data)
		{
			...
			do {
				errno = 0;
				rlen = ::SSL_write(m_ssl, reinterpret_cast< const char* >(&data[static_cast< int >(offset)]), static_cast< int >(data.size() - offset));
