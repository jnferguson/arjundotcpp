    ...
    public:
        ...
        connect(const std::string& h, const std::string& p)
        {
            ...
            for (rp = sinfo; rp != nullptr; rp = rp->ai_next) {
                ...
                ret = ::connect(m_sock, rp->ai_addr, static_cast< int >(rp->ai_addrlen));
        ...
        read(std::vector< uint8_t >& data, std::size_t len = 8192)
        {
            ...
            do {
                rlen = ::recv(m_sock, reinterpret_cast< char* >(&data[static_cast< int >(offset)]), static_cast< int >(len - offset), 0);
                ...
                if (std::numeric_limits< int >::max() <= offset)
                    return false;
        ...
        write(std::vector< uint8_t >& data)
        {
            ...
            do {
                errno = 0;
                rlen = ::send(m_sock, reinterpret_cast< const char* >(&data[static_cast< int >(offset)]), static_cast< int >(data.size() - offset), 0);
