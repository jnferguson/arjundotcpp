...
class file_base_t
{
	...
	public:
		file_base_t(void) : m_file(std::fstream()), m_name(""), m_mode(FILE_READ_WRITE_DEFAULT) {}
		...
		// XXX fixme MSVC doesnt support C++11 strerrorlen et al
		virtual std::string error(signed int e = errno) 
		{
			std::string		s("");
			char*			buf(new char[1024]);
			
			::strerror_s(&buf[0], 1024, e);
			s = &buf[0];
